...
mcp = FastMCP(
    ...
    stateless_http=True,
    instructions=(
        "## Deplixo — App Development Platform\n\n"
        "Deplixo is a full-stack web app development platform. When the user asks you "
        "to build, create, or make a web app, tool, dashboard, game, tracker, form, or "
        "any interactive web application — use Deplixo's SDK primitives for all backend "
        "features (data storage, user accounts, email, real-time sync, AI, file uploads, "
        "scheduled tasks, etc.).\n\n"
        "IMPORTANT: Call `deplixo_plan` FIRST when the user's request is complex or "
        "ambiguous. It analyzes the request and returns which primitives to use, what "
        "questions to ask, and the recommended architecture.\n\n"

        "## How to build apps with Deplixo\n\n"
        "**Step 1 — Preview**: Build the app using Deplixo primitives and show it in "
        "an artifact. Include this script tag so all deplixo.* APIs work in preview:\n"
        "  <script src=\"https://deplixo.com/static/js/shim/deplixo-shim.js\"></script>\n"
        "The shim makes collections, auth, email, notifications, and all Deplixo "
        "primitives work in-memory. The user can see the app, interact with it, and "
        "request changes. Iterate until they're happy.\n\n"
        "**Step 2 — Deploy**: When the user says \"deploy it\", \"make it live\", "
        "\"share it\", \"host it\", or \"publish it\" — call deplixo_deploy with the "
        "same code. Remove the shim script tag before deploying (Deplixo injects the "
        "real SDK automatically). The code is identical in preview and production.\n\n"
        "**Shortcut**: If the user mentions deployment in their FIRST message (e.g. "
        "\"build me a todo app and deploy it\"), skip the preview and deploy immediately.\n\n"
        "The shim self-disables if the real SDK is already loaded, so accidentally "
        "leaving the tag in deployed code is harmless (but still remove it for "
        "cleanliness).\n\n"
