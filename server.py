...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- App needs multiplayer / turn-based game -> use deplixo.rooms.join() + room.collection() + room.presence + room.broadcast\n"
        ...
        "  -> Use deplixo.rooms.join(roomId) + room.collection() + room.presence\n"
        ...
        "  // 2. Track who's in the room\n"
        "  room.presence.join({ name: playerName });\n"
        "  room.presence.onChange(({ action }) => {\n"
        "    room.presence.list().then(users => {\n"
        "      if (users.length === 2) startGame();\n"
        "    });\n"
        "  });\n\n"
        "  // 3. Use room-scoped collection for persistent game state\n"
        "  //    (scores, board, whose turn, round number)\n"
        ...
        "- Use room.presence to detect when enough players join to start\n"
